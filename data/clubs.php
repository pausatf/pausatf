...
<title>PAUSATF Club List</title><?php
  ...
  function get_db_param($param) {
    ...
    $result = mysqli_query($con, $sql) or fail_sql('get_db_param', $sql);
    $row = mysqli_fetch_row($result);
    return($row[0]);  
  }

  function fail_sql($func, $sql) {
    die("<pre>Query failed in '$func'\n$sql\n" . ((is_object($con)) ? mysqli_error($GLOBALS["___mysqli_ston"]) : (($___mysqli_res = mysqli_connect_error()) ? $___mysqli_res : false)). "</pre>");
  }
  ...
  function get_form_values() {
    foreach ($this->form_vars as $form_var => $possible_values) {
      ...
      if(isset($_REQUEST[$form_var]) && (($val = $_REQUEST[$form_var]) != '')) {
        $val = ((isset($GLOBALS["___mysqli_ston"]) && is_object($GLOBALS["___mysqli_ston"])) ? mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $val) : ((trigger_error("[MySQLConverterToo] Fix the mysql_escape_string() call! This code does not work.", E_USER_ERROR)) ? "" : ""));  // only needed for real expressions, do for all anyway
...
';
  ...
  function show_list_of_all_clubs() {
    ...
    $sql = "select tblCLUBS.club_no, club_name, roster_count, focus, locations, primary_contact, primary_phone\n" .
           ...
           $order_clause;
    $result = mysqli_query($GLOBALS["___mysqli_ston"], $sql) or fail_sql('show_all', $sql);
...
***/
  ...
  function get_club_info($club_no) {
    ...
    $result = mysqli_query($GLOBALS["___mysqli_ston"], $sql) or fail_sql('get_club_info', $sql);
