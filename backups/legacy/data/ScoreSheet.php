<?php
  ...
  /* Script for submitting scoresheets.  Written by Jeff Teeters, June 2007 */
  ...
  function initialize_timezone() {
    ...
    $result = mysqli_query($sql) or fail_sql('initialize_timezone', $sql);
  }
  ...
  function db_select1($sql) {
    ...
    $result = mysqli_query($sql) or fail_sql('db_select1', $sql);
  ...
  function get_race_and_club() {
    ...
    $output = mk_header($title, $heading) . mk_form_start('cmd')
      ...
      . "the scorer.<br />\n"
      . "Road scorer is <A href=\"mailto:tlbernhard@att.net\">Tom Bernhard</A>, "
...
****/
  ...
  function mk_race_select($time_span = 'recent') {  // use any for selecting any race
    ...
    $result = mysqli_query($sql) or fail_sql('select_race', $sql);
  ...
  function mk_club_select() {
    ...
    $sql = "select club_no, club_name from tblCLUBS where paid = 'Y' order by club_name";
    $result = mysqli_query($sql) or fail_sql('main_race', $sql);
    $options = array();
  ...
  function divisions() {
    $divisions = array(
       ...
       array('M-40', "Men's Masters"),
       array('F-40', "Women's Masters"),
       array('M-50', "Men's Seniors"),
       array('F-50', "Women's Seniors"),
       array('M-60', "Men's Super-Seniors"),
       array('F-60', "Women's Super-Seniors"),
       array('M-70', "Men's Veterans"),
       array('F-70', "Women's Veterans")
    );
	return($divisions);
  }
...
';
  ...
  function get_roster_list_box($race_number, $club_number, $age, $sex) {
    ...
    $result = mysqli_query($sql) or fail_sql('get_roster_list_box', $sql);
...
****/
  ...
  function safe_num($num, $append_coma = 1) {
    $safe = mysqli_real_escape_string($num);
    if($append_coma) {
      $safe .= ',';
    }
    return($safe);
  ...
  function safe_str($str, $append_coma = 1) {
    $safe = "'" .mysqli_real_escape_string($str). "'";
    if($append_coma) {
      $safe .= ',';
    }
    return($safe);
...
//     $mail_log = mail_user($to_all, $subject, $message, $from, false, $test);
    ...
    if($test) {
      ...
      $output = mk_header($title, $heading)
        ...
        . "<br />Road scorer is <A href=\"mailto:tlbernhard@att.net\">Tom Bernhard</A>, "
  ...
  function show_score_sheets() {
    ...
    $result = mysqli_query($sql) or fail_sql('show_score_sheets', $sql);
    $num_rows = mysqli_num_rows($result);
    if($num_rows == 0) {
...
/************* Error and logging routines **************/
  ...
  function send_mail($to, $subject, $message, $from, $cc='', $html=false, $test_mode=false) {
    ...
    if(!$test_mode) {
      ...
      // ** Don't check for error (lines below commented out). **
      // ** Reason: Not reliable on new server.  **
      // ** Change made by Jeff Teeters, 3-31-2011
      // if(!$sent) {
