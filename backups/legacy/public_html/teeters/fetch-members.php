<?php
   ...
   **/
  // define('SECURE_DIR', '../../private/');
  define('SECURE_DIR', '/var/www/legacy/private/');
  require_once (SECURE_DIR . 'usatf-national-account.php');

$dbh = 0;

  function initialize_db(){
     // this done after getting national records, because that can take some time and db connection can time out
     global $dbh;
     if(!$dbh) {
  ...
  function fetch_page($url, $method='GET', $form_data = '') {
    ...
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);  /* Fix by Thomas Vincent, 6/1/2012 */
    if($cookies) {
  ...
  function initialize_timezone() {
    ...
    date_default_timezone_set('America/Los_Angeles'); // probably not needed
  }

  function initialize_db_timezone() {
    putenv("TZ=US/Pacific");  // set php timezone
  ...
  function log_to_file($msg) {
    ...
    if(substr($program_name, -3) != 'php') {
      die("Program name does not have php extension: '$program_name'\n$msg\n" 
          . "_SERVER has: " . print_r($_SERVER, true) . "\n");
    }
  ...
  function fail($summary, $details='') {
    initialize_db();  // in case db not initialized
  ...
  function get_national_recs() {
    ...
    for ($i = 1; $i < count($lines); $i++) {
      ...
      if(isset($records[$id])) {
        ...
        if($is_current_year) {
          ...
          $saved_rec[$idx['date_applied']] = $date_applied;  // Replace next years date_applied with this years.
                                                               // date_applied always corresponds to current years membership
        } else {
          // saved_rec should be this year
          if($saved_rec[$idx['next_years_number']] != '' || $saved_rec[$idx['membership_number']] == '') {
            fail("Save record not this year pattern", "saved_rec=" . join("\t", $saved_rec) .
             "\nNew_rec=$line" . "\nsaved_rec: next_year=" . $saved_rec[$idx['next_years_number']] .
             " membership_number=" . $saved_rec[$idx['membership_number']] );
          }
    ...
    $records_ref = &$records;
    return(array($records_ref, $roster_counts));
  }
  ...
  function main() {
     initialize_timezone();
     list($national_recs, $roster_counts) = get_national_recs();
     initialize_db();
     initialize_db_timezone();
