<?php
  /* $Id: fetch-members.php,v 1.5 2007/11/02 17:44:07 Owner Exp $ */

  /* Program to fetch pausatf membership info from national USATF website
     and load it into local pausatf database table (pa_members).
     Written by Jeff Teeters, January 2007 */

  error_reporting(E_ALL);    
  require_once ('/home/pausat/db.php');  // makes $dbh database handle
  ...
  function advanced_search_form_data() {
    ...
    $form_data = "membershipNumber=&show_year=ON&" .
                 ...
                 "&show_birthDate=ON&show_birthDateVerified=ON&show_club=ON&show_dateApplied=ON&sortOrder1=lastName" .
  ...
  function field_names() {
    $field_names = "ID	Year	Membership Number	First Name	MI	Last Name	Suffix	City	State	" .
                   "Gender	Birth Date	DOB Verified	Club Affiliation	Date Applied";
    return($field_names);
  }
...
/***** No longer used, replace with get_membershipYears
date('Y');

  function get_membershipYear($page = '') {
    // extracts default selected membershipYear from advanced search form (page 'index_advanced.asp').
    // This used to build the query string for that page.  Also it is saved in the
    // pa_member_db_status table so the year can be displayed on the PA membership web pages.
    static $membershipYear = '';
    if(!$membershipYear) {
      $pattern = '#Membership Year:.*?membershipYear.*?<option value="(\d\d\d\d)" selected="selected">#s';
      if(!preg_match($pattern, $page, $matches)) {
        fail("Unable to extract membership year.", "Page is:\n$page");
      }
      $membershipYear = $matches[1];  // save for future calls.
    }
...
*******/
  ...
  function &fetch_membrRpt_txt() {
    ...
    $form_data = 'associationNumber=38&userID=dpreston&userPassword=pres38ton&submit=I+agree+-+Log+in';
  ...
  function &get_national_recs() {
    ...
    $fields = array('id', 'membership_number', 'next_years_number', 'first_name', 'middle_initial', 'last_name',
        'suffix', 'city', 'state', 'gender', 'birth_date', 'DOB_Verified', 'club_affiliation', 'date_applied');
    ...
    for ($i = 1; $i < count($lines); $i++) {
      ...
      list($id, $year, $membership_number, $first_name, $middle_initial, $last_name, $suffix, $city,
        $state, $gender, $birth_date, $DOB_Verified, $club_affiliation, $date_applied) = explode("\t", $line);
      ...
      $is_current_year = $year == $membershipYears[0];  // if false, then next year
      if(isset($records[$id])) {
        ...
        if($is_current_year) {
          ...
          $refresh_fields = array('first_name', 'middle_initial', 'last_name', 'suffix', 'city', 'state',
            'gender', 'birth_date', 'DOB_Verified', 'club_affiliation', 'date_applied');
  ...
  function &get_pa_recs() {
    ...
    $sql = "select id, membership_number, next_years_number, first_name, middle_initial, last_name, suffix, city,\n" .
        "state, gender, birth_date, DOB_Verified, club_affiliation, date_applied\n" .
  ...
  function update_db(&$national_recs, &$pa_recs) {
    ...
    if($all_adds) {
      ...
      $sql = "insert into pa_members\n" .
        ...
        "state, gender, birth_date, DOB_Verified, club_affiliation, date_applied) values\n" .
        implode(",\n", $inserts);
      $result = mysql_query($sql) or fail_sql('update_db', $sql);
    }
