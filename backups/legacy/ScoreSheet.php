...
****/
  ...
  function mk_race_select($time_span = 'recent') {  // use any for selecting any race
    ...
    $result = mysql_query($sql) or fail_sql('select_race', $sql);
  ...
  function mk_club_select() {
    ...
    $result = mysql_query($sql) or fail_sql('main_race', $sql);
    $options = array();
...
';
  ...
  function get_roster_list_box($race_number, $club_number, $age, $sex) {
    ...
    $result = mysql_query($sql) or fail_sql('get_roster_list_box', $sql);
...
****/
  ...
  function safe_num($num, $append_coma = 1) {
    $safe = mysql_real_escape_string($num);
    if($append_coma) {
      $safe .= ',';
    }
    return($safe);
  ...
  function safe_str($str, $append_coma = 1) {
    $safe = "'" .mysql_real_escape_string($str). "'";
    if($append_coma) {
      $safe .= ',';
    }
    return($safe);
...
//     $mail_log = mail_user($to_all, $subject, $message, $from, false, $test);
  ...
  function show_score_sheets() {
    ...
    $result = mysql_query($sql) or fail_sql('show_score_sheets', $sql);
    $num_rows = mysql_num_rows($result);
    if($num_rows == 0) {
