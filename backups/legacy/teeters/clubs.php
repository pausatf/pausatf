<?php
  ...
  function this_script_name_base() {
    // e.g. 'clubs' rather than 'clubs.php'
    $this_script_name = this_script_name();
    $pos = strripos($this_script_name, '.php');
    if ($pos === false) {
      die ("Could not find '.php' at end of script name '$this_script_name'");
    }
    $name_base = substr($this_script_name, 0, $pos);
    return $name_base;
  }

  function get_template($template_suffix) {
    $template_name_base = get_request('tpl');
    if(!$template_name_base) {
      $template_name_base = this_script_name_base();
    }
    $template_name = $template_name_base . $template_suffix . ".html";
    $template = file_get_contents($template_name);
    if(false === $template) {
      die("Could not find template file '$template_name'");
    }
    return $template;
  }

  function merge_template($dict) {
    // parse the template
    $pattern = '#(.*?)<(div|span)\s+type\s*=\s*"?fill-by-php"?[^>]*>([^<]+)</\2>#is';
    $template = get_template($dict['template_suffix']);
    $nfound = preg_match_all($pattern, $template, $matches, PREG_OFFSET_CAPTURE + PREG_SET_ORDER);
    if($nfound === false) {
      die("Error in preg_match_all scan of template");
    }
    if($nfound == 0) {
      die("No matches in template");
    }
    // print "<pre>Found $nfound matches\n" . print_r($matches). "</pre>";
    //  merge template and $dict
    $output = '';
    foreach ($matches as $match) {
      $template_text = $match[1][0];
      $dict_key = $match[3][0];
      $offset = $match[3][1];  // only last one needed, after loop
      if(!isset($dict[$dict_key])) {
        die("Invalid template key: '$dict_key', should be one of: [" . implode(', ', array_keys($dict)) . "]");
      }
      $output .= $template_text . $dict[$dict_key];
    }
    // Now need to append final part of template
    // first, extract final part of template
    $pattern = "#$dict_key</(div|span)>(.*)#is";
    $nmatches = preg_match($pattern, $template, $matches, 0, $offset);
    if($nmatches == 0) {
      die("Could not find ending part of template");
    }
    $end_template = $matches[2];
    $output .= $end_template;
    return $output;
  }
...
';
  ...
  function show_list_of_all_clubs() {
    ...
    $all_clubs_table = start_all_clubs_table($oi);
...
<TR>
    ...
    $all_clubs_table .= finish_all_clubs_table();
    $dict = array('template_suffix' => '',
                  'page_title' => "List of all clubs",
                  'all_clubs_table' => $all_clubs_table);
    return $dict;
  }
...
***/
  ...
  function show_club_info($club_no) {
    ...
    $dict = array('template_suffix' => '-one',
