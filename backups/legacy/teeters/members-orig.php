...
return($html);
  ...
  function custom_css($id, $x_offset=-50, $y_offset=10) {
...
class Option_information {
  ...
  function get_age_explain() {
     $html = "<div style=\"text-align:center;\"><b style=\"color: blue;\">Age Column:</b></div>\n"
            . "If the age is <b>bold</b> and <u>underlined</u><br />\n"
            . "then the date of birth has been<br />\n"
            . " verified, otherwise it has not.<br />\n"
	    . "Examples:\n<br />\n"
            . "&nbsp;&nbsp;<b><u>27</u></b> -- DOB Verified.<br />\n"
  ...
  function cmd_members() {
    ...
    if($showing_renewals) {
      ...
      $renewal_explain = "'Y' means has " . $next_year . " membership";
      $next_year_header = "<a href=\"#\">$next_year_dd <span>$renewal_explain</span></a>"; 
    } else {
      $renewed_clause = '';
    }
    ...
    while($row = mysql_fetch_array($result)) {
      ...
      if($club_specified) {
        if(strtoupper($row['paid']) != 'Y' && $row['club_no'] != 0) {
