...
function et_core_autoloader( $class_name ) {
	if ( ! str_starts_with( $class_name, 'ET_Core' ) ) {
		return;
	}
...
function et_core_security_check( $user_can = 'manage_options', $nonce_action = '', $nonce_key = '', $nonce_location = '_POST', $die = true ) {
	...
	if ( empty( $nonce_key ) && ! str_contains( $nonce_action, '_nonce' ) ) {
...
function et_core_setup( $deprecated = '' ) {
	...
	if ( str_starts_with( $core_path, $theme_dir ) ) {
...
function et_core_load_component( $components ) {
	...
	$is_jetpack = isset( $_SERVER['HTTP_USER_AGENT'] ) && str_contains( $_SERVER['HTTP_USER_AGENT'], 'Jetpack' );
...
function et_get_attachment_id_by_url( $url ) {
	...
	if ( ! $attachment_id && str_contains( $normalized_url, '-scaled.' ) ) {
	...
	if ( ! $attachment_id ) {
		...
		if ( str_starts_with( $normalized_url, $uploads_baseurl ) ) {
...
function et_get_attachment_size_by_url( $url, $default_size = 'full' ) {
	...
	if ( str_ends_with( $url, $metadata['file'] ) ) {
...
function et_attachment_normalize_url( $url ) {
	...
	if ( ! str_starts_with( $url, 'http' ) ) {
		...
		if ( str_starts_with( $url_trimmed, $upload_dir ) || str_starts_with( $url_trimmed, 'wp-content' ) ) {
...
function et_core_get_post_builder_meta( $post_id ) {
	...
	foreach ( $raw_meta as $key => $values ) {
		if ( ! str_starts_with( $key, '_et_pb_' ) && ! str_starts_with( $key, '_et_builder_' ) ) {
			continue;
		}

		if ( str_starts_with( $key, '_et_pb_ab_' ) ) {
