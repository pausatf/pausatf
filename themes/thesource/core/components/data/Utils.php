...
class ET_Core_Data_Utils {
	 ...
	 *
	 * @since ?? Don't execute deprecated `libxml_disable_entity_loader()` on PHP 8.0.
	 *
	 * @param bool $disable
	 *
	 * @return void
	...
	public function remove_empty_directories( $path ) {
		...
		if ( ! str_starts_with( $path, $content_dir ) || $content_dir === $path ) {
			return;
		}

		$capability = str_starts_with( $path, "{$content_dir}/cache/et" ) ? 'edit_posts' : 'manage_options';
	...
	public function array_transform( $data, $data_map, $direction = '->', $exclude_keys = array() ) {
		...
		foreach ( $data_map as $address_1 => $address_2 ) {
			...
			if ( str_starts_with( $to_address, '@' ) || str_starts_with( $from_address, '@' ) ) {
				$array_value_required = true;
				$to_address           = ltrim( $to_address, '@' );
				$from_address         = ltrim( $from_address, '@' );

			} else if ( str_starts_with( $to_address, '!' ) || str_starts_with( $from_address, '!' ) ) {
	...
	public function sanitize_css_placeholders( $selector ) {
		...
		foreach ( $selectors as $_selector ) {
			foreach ( $placeholders as $placeholder ) {
				if ( str_contains( $_selector, $placeholder ) ) {
