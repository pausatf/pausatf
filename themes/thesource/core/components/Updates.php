<?php
/**
 * PAUSATF TheSource theme source: Updates.php.
 *
...
final class ET_Core_Updates {
	...
	function replace_theme_update_notification( $themes_array ) {
		...
		foreach ( $themes_array as $id => $theme_data ) {
			...
			if (
				...
				&& str_contains( $theme_data['update'], 'Automatic update is unavailable for this theme' )
			) {
	...
	function change_plugin_changelog_url( $url, $path ) {
		if ( ! str_starts_with( $path, 'plugin-install.php?tab=plugin-information&plugin=' ) ) {
			return $url;
		}
