<?php
/**
 * PAUSATF TheSource theme source: custom_functions.php.
 *
...
if ( ! function_exists( 'et_sync_custom_css_options' ) ) {
	function et_sync_custom_css_options() {
		...
		if ( $wp_custom_css === $legacy_custom_css || str_contains( $wp_custom_css, $legacy_custom_css ) ) {
			set_theme_mod( 'et_pb_css_synced', 'yes' );
			return;
		}
...
if ( ! function_exists( 'et_path_reltoabs' ) ) {
	...
	function et_path_reltoabs( $imageurl ){
		if ( str_contains( strtolower( $imageurl ), 'http://' ) || str_contains( strtolower( $imageurl ), 'https://' ) ) return $imageurl;

		if ( str_contains( strtolower( $imageurl ), $_SERVER['HTTP_HOST'] ) )
			return $imageurl;
		else {
...
function head_addons(){
	...
	// Disabled: child theme enqueues style-Red.css via wp_enqueue_style.
...
// Disabled: WordPress title-tag + Jetpack SEO handles titles. // add_filter( 'pre_get_document_title', 'elegant_titles_filter' );
...
if ( ! function_exists( 'et_resize_image' ) ) {
	...
	function et_resize_image( $thumb, $new_width, $new_height, $crop ){
		 ...
		 */
		if ( ! str_contains( $thumb, $site_uri ) )
			return $thumb;
	...
	function et_gf_enqueue_fonts( $et_gf_font_names ) {
		...
		foreach ( $et_gf_font_names as $et_gf_font_name ) {
			...
			if ( 'false' === et_get_option( "{$shortname}_gf_enable_all_character_sets", 'false' ) ) {
				$latin_ext = '';
				if ( str_contains( $google_fonts[$et_gf_font_name]['character_set'], 'latin-ext' ) )
				$latin_ext = ',latin-ext';
