...
**What it checks:**
- ✅ Production server backup status (droplet REDACTED_DROPLET_ID)
- ✅ Staging server backup status (droplet REDACTED_DROPLET_ID)
...
- `PROD_DROPLET_ID` - Production droplet ID (REDACTED_DROPLET_ID)
- `STAGING_DROPLET_ID` - Staging droplet ID (REDACTED_DROPLET_ID)
...
```
Production Server:
ID: REDACTED_DROPLET_ID, Memory: 8192 MB, VCPUs: 4, Disk: 160 GB, Status: active

Staging Server:
ID: REDACTED_DROPLET_ID, Memory: 4096 MB, VCPUs: 2, Disk: 80 GB, Status: active
```
...
**Allowed/Expected:**
- Server IPs: REDACTED_PROD_NEW_IP (production), REDACTED_STAGE_IP (staging)
