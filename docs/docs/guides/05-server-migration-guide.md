...
Droplet ID:   REDACTED_DROPLET_ID
...
Current Records:
  www.pausatf.org  -> REDACTED_PROD_NEW_IP (A record)
  prod.pausatf.org  -> REDACTED_PROD_NEW_IP (A record)
...
doctl compute droplet-action shutdown REDACTED_DROPLET_ID --wait

# Create snapshot
doctl compute droplet-action snapshot REDACTED_DROPLET_ID \
  --snapshot-name "pausatf-pre-migration-$(date +%Y%m%d)" \
  --wait

# Power back on
doctl compute droplet-action power-on REDACTED_DROPLET_ID --wait
...
# From new server:
rsync -avz --progress \
  root@REDACTED_PROD_NEW_IP:/var/www/legacy/public_html/ \
  /var/www/legacy/public_html/
...
cd /root/migration-restore
scp root@REDACTED_PROD_NEW_IP:/root/migration-backup/apache-configs-*.tar.gz .
scp root@REDACTED_PROD_NEW_IP:/root/migration-backup/php-configs-*.tar.gz .
scp root@REDACTED_PROD_NEW_IP:/root/migration-backup/custom-scripts-*.tar.gz .
...
cd /root/migration-restore
scp root@REDACTED_PROD_NEW_IP:/root/migration-backup/database-*.sql.gz .
...
doctl compute droplet delete REDACTED_DROPLET_ID
...
# Update Cloudflare DNS back to old IP: REDACTED_PROD_NEW_IP
curl -X PUT "https://api.cloudflare.com/client/v4/zones/${ZONE_ID}/dns_records/RECORD_ID" \
  -H "Authorization: Bearer ${API_TOKEN}" \
  -H "Content-Type: application/json" \
  --data '{"type":"A","name":"www.pausatf.org","content":"REDACTED_PROD_NEW_IP","ttl":300,"proxied":true}'
