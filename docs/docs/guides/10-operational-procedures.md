...
doctl compute droplet-action snapshot REDACTED_DROPLET_ID --snapshot-name "pre-plugin-update-$(date +%Y%m%d)"
...
doctl compute droplet backup list REDACTED_DROPLET_ID  # Production
doctl compute droplet backup list REDACTED_DROPLET_ID  # Staging
...
doctl compute droplet get REDACTED_DROPLET_ID  # Check Features column
doctl compute droplet get REDACTED_DROPLET_ID

# List available backups (should see daily backups)
doctl compute droplet backup list REDACTED_DROPLET_ID
doctl compute droplet backup list REDACTED_DROPLET_ID
```
...
```bash
# Power on droplet
doctl compute droplet-action power-on REDACTED_DROPLET_ID

# Reboot droplet
doctl compute droplet-action reboot REDACTED_DROPLET_ID
```
