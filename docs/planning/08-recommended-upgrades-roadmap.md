...
1. **Create full backup:**
   ...
   doctl compute droplet-action snapshot REDACTED_DROPLET_ID --snapshot-name "pre-php83-upgrade-$(date +%Y%m%d)"
...
```bash
# Backup first!
doctl compute droplet-action snapshot REDACTED_DROPLET_ID --snapshot-name "pre-os-upgrade"
...
doctl compute droplet-action restore REDACTED_DROPLET_ID --snapshot-id <snapshot-id>
...
- Ubuntu 20.04 downloads and release notes: https://releases.ubuntu.com/20.04/
- Ubuntu release support status and dates: https://ubuntu.com/project/docs/release-team/list-of-releases/
