...
2. **Create new droplet from backup**
   ...
   doctl compute droplet-action list REDACTED_DROPLET_ID

   # Restore from most recent backup
   doctl compute droplet-action restore REDACTED_DROPLET_ID --image-id BACKUP_IMAGE_ID
   ```
...
doctl compute droplet-action snapshot REDACTED_DROPLET_ID --snapshot-name "test-backup-$(date +%Y%m%d)"
