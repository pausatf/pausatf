...
# Create snapshot
doctl compute droplet-action snapshot REDACTED_DROPLET_ID --snapshot-name "pre-upgrade-$(date +%Y%m%d)"

# List backups
doctl compute droplet backups REDACTED_DROPLET_ID
```

### Via API
```bash
# Get droplet
curl -X GET "https://api.digitalocean.com/v2/droplets/REDACTED_DROPLET_ID" \
