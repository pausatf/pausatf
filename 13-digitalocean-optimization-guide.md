...
**Current:** Staging server (REDACTED_DROPLET_ID) has NO firewall
...
**Current:** Direct IP (REDACTED_PROD_NEW_IP) used for prod.pausatf.org
...
doctl compute floating-ip-action assign <FLOATING_IP> REDACTED_DROPLET_ID
```
...
# Verify installation
doctl compute droplet get REDACTED_DROPLET_ID | grep droplet_agent
doctl compute droplet get REDACTED_DROPLET_ID | grep droplet_agent
```
  ...
  create-snapshot:
    ...
    steps:
      ...
      - name: Create Snapshot
        run: |
          DROPLET_ID="${{ github.event.inputs.server == 'production' && 'REDACTED_DROPLET_ID' || 'REDACTED_DROPLET_ID' }}"
    ...
    steps:
      ...
      - name: Generate Report
        run: |
          ...
          doctl compute droplet backup list REDACTED_DROPLET_ID | head -5 >> report.md
